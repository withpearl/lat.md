...
Git projects enumerate their tracked regular source files and the untracked files Git does not ignore, once, excluding symlinks, sources beneath dot-directories, dependency trees, the root vault, and nested Lat projects, then give that identical ordered list to ripgrep or the TypeScript scanner. Ignored build output therefore never enters project validation, while a source file nobody has staged yet — a test written moments ago — counts at once.
