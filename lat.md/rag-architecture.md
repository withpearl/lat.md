...
[[src/search/cache.ts#writeIndex]] serializes writers with a process-owned lock, copies the active generation for incremental work, checkpoints the result, writes a compacted copy with `VACUUM INTO`, and atomically replaces `search-index.json` with the compacted file. The engine does not reuse pages freed by updates, and every FTS rebuild frees the old index, so uncompacted generations grow with each publish; if compaction fails the checkpointed staging file is published instead. Staging uses single-process access; published database opens enable experimental multiprocess WAL on Unix. Windows uses the supported default WAL mode. Windows search and publication metadata readers open private temporary copies of checkpointed generations. FTS requires writable handles, so copying avoids exclusive locks on the published file. Writers mutate staging files rather than checkpointing the active generation; readers keep their original snapshot until closed.

Unchanged work discards its staging copy. Failed work removes staging files and preserves the manifest. Each publish deletes generations older than the one it replaces, which stays available for existing readers.
...
Tests: [[tests/search#Hybrid Retrieval#Publishes only successful generations]], [[tests/search#Hybrid Retrieval#Publishes compacted generations]], [[tests/search#Hybrid Retrieval#Publishes uncompacted when compaction fails]], [[tests/search#Hybrid Retrieval#Preserves FTS rollback and portable copies]], [[tests/search#Hybrid Retrieval#Archives legacy caches without overwriting backups]], and [[tests/search#Hybrid Retrieval#Keeps readers alive across process boundaries]].
